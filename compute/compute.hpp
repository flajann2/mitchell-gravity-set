...
template <typename F = double> struct Scalar {
  F value;
  explicit Scalar(F v) : value(v) {}
};

template <typename F = double> struct Coords {
  std::vector<F> vec;
...
template <typename F = double, typename I = int> struct Field {
